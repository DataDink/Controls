...
{
    public class ObservableList<T> : IList, IList<T>, INotifyCollectionChanged
    {
        ...
        protected ObservableList(List<T> wrapper)
        {
            Internal = wrapper ?? new List<T>();
        }
        ...
        public ObservableList<TOutput> ConvertAll<TOutput>(Converter<T, TOutput> converter)
        {
            ...
            var obsv = new ObservableList<TOutput>() { Internal = intl };
            return obsv;
        }
        ...
        public ObservableList<T> FindAll(Predicate<T> match)
        {
            ...
            var obsv = new ObservableList<T> { Internal = intl };
            return obsv;
        }
        ...
        /// <summary>
        /// WARNING: This method has a higher than normal performance hit.
        /// </summary>
        public int RemoveAll(Predicate<T> match)
        {
            // ouch... the predicate must only be run once per item, and both the kept and discarded items must be tracked for notification.
            var keep = new List<T>();
            var discard = new List<T>();
            Internal.ForEach(t => { if (match(t)) keep.Add(t); else discard.Add(t); });
            Internal.Clear();
            Internal.AddRange(keep);
            OnCollectionChanged(new NotifyCollectionChangedEventArgs(NotifyCollectionChangedAction.Remove, discard));
